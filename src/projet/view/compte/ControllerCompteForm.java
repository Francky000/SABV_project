...
public class ControllerCompteForm {
	...
	private void initialize() {
		...
		enCours = modelCompte.getEnCours();

		// Data binding
		enCours = modelCompte.getEnCours();
		textFieldId.textProperty().bindBidirectional( enCours.idProperty(), new IntegerStringConverter());
		textFieldPseudo.textProperty().bindBidirectional( enCours.pseudoProperty() );
		textFieldMotDePasse.textProperty().bindBidirectional( enCours.motDePasseProperty() );
		textFieldEmail.textProperty().bindBidirectional( enCours.emailProperty() );
    ...
    private void actualiserListeItemRoles() {
		...
		for( ItemRole item : itemRoles ) {
			item.setChoisi( enCours.isInRole( item.getRole() ) );
		}
    }
		...
		public ItemRole( String role, boolean present ) {
    		...
    		choisi.addListener(
    			(ChangeListener<Boolean> ) ( obs, oldValue, newValue ) -> {
    				if ( newValue ) {
    					if( ! enCours.getRoles().contains( role) ) {
        					enCours.getRoles().add( role );
    					}
    				} else {
       					enCours.getRoles().remove( role );
    				}
        		});
		}
